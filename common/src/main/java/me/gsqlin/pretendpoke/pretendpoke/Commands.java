...
public class Commands implements CommandExecutor , TabCompleter {
    String[] help = new String[]{
            "§7["+ CommonData.plugin.getDescription().getPrefix() +"]      命令头↓",
            "§7子命令↓                §r ptendp",
            "§7--reload           §r重载配置",
    ...
    public boolean onCommand(CommandSender sender,Command command,String label,String[] args) {
        if (!CommonData.plugin.getConfig().getBoolean("开启原功能")){
        ...
        if (args.length>=1){
            ...
            if (args[0].equalsIgnoreCase("reload")){
                ...
                CommonData.plugin.reloadConfig();
                ...
                List<String> list = CommonData.plugin.getConfig().getStringList("玩家数据."+name+".可伪装");
                int x = CommonData.plugin.getConfig().getInt("每种精灵抓多少次才能伪装");
                if (args.length >= 2){
                    String pokeName = args[1];
                    if (x == 0){
                        if (PretendHelper.startPretend(p,pokeName)){
                            p.sendMessage("§7开始伪装");
                        }else{
                    ...
                    if (list.contains(pokeName)){
                        if (CommonData.plugin.getConfig().getBoolean("世界配置.启用")&&
                        !CommonData.plugin.getConfig().getStringList("世界配置.worlds").contains(p.getWorld().getName())){
                ...
                if (!PretendHelper.runnableMap.containsKey(p.getName())){
                    p.sendMessage("§7你没有伪装呢~");
                    return false;
                }
                if (PretendHelper.stopPretend(p)){
    ...
    public List<String> onTabComplete(CommandSender sender,Command command,String label,String[] args) {
        ...
        if (args.length >= 1) {
            if (args[0].equalsIgnoreCase("pretend")){
                List<String> canPre = CommonData.plugin.getConfig().getStringList("玩家数据."+((Player)sender).getName()+".可伪装");
                if (CommonData.plugin.getConfig().getInt("每种精灵抓多少次才能伪装") == 0){
                    canPre = CommonData.versionHelper.getCanPretendPokeNameList();
                }
